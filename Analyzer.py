...
# Use the percentiles as the ground reference
z_lo, z_hi = np.percentile(Z, [20, 80])
inlier_mask = (Z >= z_lo) & (Z <= z_hi)
A = np.c_[XY[inlier_mask], np.ones(np.sum(inlier_mask))]
C, _, _, _ = np.linalg.lstsq(A, Z[inlier_mask], rcond=None)
Z_pred = np.c_[XY, np.ones(XY.shape[0])] @ C
residuals = Z - Z_pred
...
plt.legend()
plt.title("Residuals Distribution (Ground = 5th percentile)")
plt.xlabel("Residual (meters)")
plt.ylabel("Count")
plt.show()
max_defect = 0.1  # 10 cm
# Classify
...
o3d.visualization.draw_geometries([pcd], window_name="Grass & Crack Detection (Percentile Ground)", width=1280, height=720)
